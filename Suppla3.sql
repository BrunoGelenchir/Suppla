...
# c:\xampp\mysql\bin\mysql.exe -uedunova -pedunova --default-character-set=utf8 < c:\suppla3.sql
...
alter table post add foreign key (supp) references supp (supp_id);
...
alter table supp add foreign key (profiles) references profiles (profiles_id);
 ...
 alter table post add foreign key (buy) references buy (buy_id);
